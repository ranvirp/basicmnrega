...
class PhotoCreateAction extends \yii\rest\Action
{
    ...
    public function run()
    {
      ...
      // $model->title=Yii::$app->getRequest()->getBodyParams();
		   ...
		   $model->created_at=$model->created_time;
        if (!$model->validate()) Yii::trace(print_r($model,true).print_r($model->errors,true), __METHOD__);
        if ($model->save()) {
            ...
            Yii::trace(print_r($model->errors,true), __METHOD__);

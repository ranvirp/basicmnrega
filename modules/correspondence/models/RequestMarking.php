 ...
 */
class RequestMarking extends \yii\db\ActiveRecord
{
   ...
   public static function mapping()
    {
     return [
               'request'=>'\app\modules\correspondence\models\Request',
    ...
    public function getRequest()
    {
       ...
       return $class::find()->where(['id'=>$this->request_id])->one();
     ...
     
      {
        ...
        // print_r($status);
         ...
         foreach ($status as $s1)
          {
          $x="SUM(CASE WHEN request_marking.flag!=1 AND request_marking.status=".$s1." and request_type='".$tc."'";
          if ($d!=-1) $x.="and receiver=".$d;
          $q[]=$x." THEN 1 ELSE 0 END) AS ".$tc."_count"."_".$s1;
          }
          $x="SUM(CASE WHEN request_marking.flag!=1 AND request_type='".$tc."'";
           if ($d!=-1) $x.="and receiver=".$d;

          $q[]=$x." THEN 1 ELSE 0 END) AS ".$tc."_count";
         
        }
        $query="SELECT ".implode(",",$q)." FROM request_marking";
     ...
     
      {
        ...
        // print_r($status);
         ...
         foreach ($flags as $flag)
          {
          $x="SUM(CASE WHEN request_marking.flag=".$flag." and request_type='".$tc."'";
          if ($d!=-1) $x.="and receiver=".$d;

...
$this->registerJs(
   '$("document").ready(function(){ 
        $("#new_parameter").on("pjax:end", function() {
            $.pjax.reload({container:"#parameters"});  //Reload GridView
        });
    });'
);
?>
<div class="bordered-form parameter-form">
  <div class="form-title">
    <div class="form-title-span">
        <span>Form for creating Parameter</span>
    </div>
</div>
    ...
    <?= $model->showForm($form,"type") ?>

    <?= $model->showForm($form,"link") ?>
    ...
    <?= $model->showForm($form,"weight") ?>

    <?= $model->showForm($form,"unit") ?>

<?php
/*
try {
$x= Utility::rules()["app\modules\mnrega\models\Parameter"][$changeattribute];
} catch (Exception $e) {$x=null;}
$modelArray=Yii::$app->request->post("Parameter");
